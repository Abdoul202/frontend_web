  ...
  const load = async () => {
    ...
    try {
      const [a, l, e] = await Promise.all([
        ...
        alertsAPI.getExpiring({ days: 60 }),
      ])
      setAlerts(a.data.data)
      setUnread(a.data.unreadCount)
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex gap-1 bg-surface-100 rounded-xl p-1 w-fit mb-6">
        {[
          ...
          { id: 'stock',    label: `Stock bas (${lowStock.length})` },
          { id: 'expiring', label: `Expirations (${expiring.length})` },
        ].map(t => (
            ...
            <div className="space-y-2">
              {alerts.length === 0 ? (
                <div className="card text-center py-12">
                  <Bell size={36} className="text-surface-200 mx-auto mb-3" />
                  <p className="text-surface-500">Aucune alerte</p>
                </div>
              ) : alerts.map(a => (
                <div key={a._id} className={`flex items-start gap-4 p-4 rounded-xl border transition-all ${a.lu ? 'bg-white border-surface-200' : 'bg-amber-50 border-amber-200'}`}>
                  ...
                  {!a.lu && (
                    <button onClick={async () => { await alertsAPI.markRead(a._id); load() }}
                      className="text-xs text-surface-400 hover:text-surface-600 shrink-0 transition-colors">
                      Marquer lu
                    </button>
                  )}
                </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-surface-50 border-b border-surface-200 text-left">
                    ...
                    <th className="px-5 py-3 font-medium text-surface-500">Stock actuel</th>
                    <th className="px-5 py-3 font-medium text-surface-500">Seuil alerte</th>
                    <th className="px-5 py-3 font-medium text-surface-500">Statut</th>
                  </tr>
                </thead>
                <tbody>
                  {lowStock.length === 0 ? (
                    <tr><td colSpan={4} className="py-12 text-center text-surface-400 text-sm">Aucun stock bas 🎉</td></tr>
                  ) : lowStock.map(m => (
                    <tr key={m._id} className="table-row">
                      <td className="px-5 py-3.5 font-medium text-surface-800">{m.nom}</td>
                      <td className="px-5 py-3.5 font-semibold text-red-600 font-mono">{m.quantiteTotale}</td>
                      <td className="px-5 py-3.5 text-surface-500">{m.seuilAlerte}</td>
                      <td className="px-5 py-3.5">
                        {m.quantiteTotale === 0
                          ? <span className="badge-red">Rupture totale</span>
                          : <span className="badge-yellow flex items-center gap-1 w-fit"><AlertTriangle size={10}/> Stock bas</span>}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          {tab === 'expiring' && (
            <div className="card p-0 overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {expiring.length === 0 ? (
                    <tr><td colSpan={5} className="py-12 text-center text-surface-400 text-sm">Aucun lot n'expire dans les 60 prochains jours 🎉</td></tr>
                  ) : expiring.map(lot => {
                    ...
                    return (
                      <tr key={lot._id} className="table-row">
                        ...
                        <td className="px-5 py-3.5">{lot.quantite}</td>
                        <td className="px-5 py-3.5 text-surface-600">{format(exp, 'dd/MM/yyyy')}</td>
                        <td className="px-5 py-3.5">
                          {isExpired ? <span className="badge-red">Périmé</span>
                            : daysLeft <= 30 ? <span className="badge-red">{daysLeft}j restants</span>
                            : <span className="badge-yellow">{daysLeft}j restants</span>}
                        </td>
                      </tr>
                    )
                  })}
