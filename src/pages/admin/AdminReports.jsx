...
export default function AdminReports() {
  ...
  const loadMonthly = async () => {
    ...
    catch { toast.error('Erreur chargement rapport') } finally { setLoading(false) }
  }
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <PageHeader title="Rapports" subtitle="Analyse financière et exports"/>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {monthly && (
            <div className="pt-4 border-t border-surface-100 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-primary-50 rounded-xl p-3 text-center"><p className="text-xl font-display font-bold text-primary-700">{monthly.totalCA?.toLocaleString()}</p><p className="text-xs text-primary-500 mt-0.5">FCFA — CA total</p></div>
                ...
                <div className="bg-surface-50 rounded-xl p-3 text-center"><p className="text-xl font-display font-bold text-surface-700">{monthly.margeGlobale?.toFixed(1)}%</p><p className="text-xs text-surface-400 mt-0.5">Marge</p></div>
                {monthly.valeurPerdue > 0 && <div className="bg-red-50 rounded-xl p-3 text-center"><p className="text-xl font-display font-bold text-red-600">{monthly.valeurPerdue?.toLocaleString()}</p><p className="text-xs text-red-400 mt-0.5">Valeur périmés</p></div>}
              </div>
              <div>
        ...
        <div className="card">
          ...
          <p className="text-sm text-surface-500 mb-6">Export Excel complet — quantités disponibles, lots actifs, valeur du stock, indicateurs périmés.</p>
          <button onClick={downloadStockXLSX} className="btn-primary w-full justify-center"><Download size={15}/> Exporter en Excel (.xlsx)</button>
        </div>
      </div>
    </div>
  )
