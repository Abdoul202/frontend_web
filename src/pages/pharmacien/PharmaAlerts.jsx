...
export default function PharmaAlerts() {
          ...
          <div className="space-y-2">
            ...
            : alerts.map(a => (
              <div key={a._id} className={`flex items-start gap-4 p-4 rounded-xl border ${a.lu ? 'bg-white border-surface-200' : 'bg-amber-50 border-amber-200'}`}>
                ...
                <div className="flex-1"><p className={`text-sm ${a.lu ? 'text-surface-700' : 'text-surface-900 font-medium'}`}>{a.message}</p><p className="text-xs text-surface-400 mt-0.5">{format(new Date(a.createdAt), 'dd/MM/yyyy HH:mm')}</p></div>
