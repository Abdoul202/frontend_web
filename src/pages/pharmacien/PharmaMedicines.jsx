...
import { Pill, Plus, Pencil, Trash2, Download, Eye } from 'lucide-react'

const FORMES = ['comprimé','gélule','sirop','injectable','pommade','suppositoire','sachet','gouttes','autre']
const EMPTY  = { nom:'', dci:'', forme:'comprimé', dosage:'', categorie:'', prixVente:'', prixAchat:'', seuilAlerte:'10', fabricant:'' }
...
export default function PharmaMedicines() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(modal === 'create' ? 'Médicament ajouté' : 'Modifié')
      setModal(null); load()
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <PageHeader title="Médicaments" subtitle="Catalogue et gestion">
      ...
      <div className="card p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            : medicines.length === 0 ? <tr><td colSpan={6}><EmptyState icon={Pill} title="Aucun médicament" action={<button onClick={openCreate} className="btn-primary">Ajouter</button>}/></td></tr>
            : medicines.map(m => (
              <tr key={m._id} className="table-row">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-1 justify-end">
                    <button onClick={() => navigate(`/medicines/${m._id}`)} className="p-1.5 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-surface-600"><Eye size={15}/></button>
                    <button onClick={() => openEdit(m)} className="p-1.5 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-surface-600"><Pencil size={15}/></button>
                    <button onClick={() => setDeleteTarget(m)} className="p-1.5 rounded-lg hover:bg-red-50 text-surface-400 hover:text-red-500"><Trash2 size={15}/></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === 'create' ? 'Ajouter un médicament' : 'Modifier'} size="lg">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div><label className="label">Seuil alerte</label><input className="input" type="number" value={form.seuilAlerte} onChange={f('seuilAlerte')}/></div>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button onClick={handleSave} disabled={saving || !form.nom || !form.prixVente} className="btn-primary">{saving ? 'Enregistrement…' : 'Enregistrer'}</button>
        </div>
      </Modal>
